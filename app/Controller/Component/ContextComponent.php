...
class ContextComponent extends Component {
		...
		function checkSession(&$controller){
			if ($controller->Session->read('User.id')==NULL) $controller->redirect('/mainsites/login');
		}
		...
		function doNotPermit(&$controller, $group){
			...
			foreach($array_groups as $group) {
				if (strtoupper($controller->Session->read('User.group')) == trim(strtoupper($group))) {
					$controller->redirect('/mainsites/select_view');
				}
			}
		}
		...
		function createUserSession(&$controller, $data = array()) {
			if (array_key_exists('User', $data)) {
				...
				$controller->Session->write('User.email', $data['User']['email']);
				$controller->Session->write('User.username', $data['User']['username']);
				$controller->Session->write('User.wholename', $data['User']['fname'].' '.$data['User']['mname'].' '.$data['User']['lname'].' '.$data['User']['ext']);
				$controller->Session->write('User.group', $data['User']['group']);
				$controller->Session->write('User.facebook', $data['User']['facebook']);
			} else {
